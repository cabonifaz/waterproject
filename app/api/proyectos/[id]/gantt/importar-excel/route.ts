// app/api/proyectos/[id]/gantt/importar-excel/route.ts
// Recibe un .xlsx (multipart/form-data, campo "archivo") — el mismo
// formato que exporta el Gantt Planificado — y sincroniza sus marcas con
// la base de datos. Solo funciona mientras el planificado está 'abierto'.
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const resultado = await importGanttExcelService.importarGanttExcel(proyectoId, buffer, 'planificado');

    return NextResponse.json({ success: true, data: resultado, timestamp: new Date().toISOString() });
  } catch (error) {
    console.error('Error importando Excel del Gantt planificado:', error);
